import axios from "axios";
import React, { Fragment, useEffect, useState } from "react";

const Weather = () => {
  const [data, setData] = useState(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null);

  useEffect(() => {
    axios
      .get(
        "https://api.openweathermap.org/data/2.5/forecast?lat=33.647562&lon=73.037891&appid=08feca854374ccaf32b556b1dbcae80a"
      )
      .then((response) => {
        console.log(response.data);
        setData(response.data);
        setLoading(false);
      })
      .catch((error) => {
        console.error("Error fetching weather data:", error);
        setError("Failed to fetch weather data");
        setLoading(false);
      });
  }, []);

  if (loading) return <div>Loading...</div>;
  if (error) return <div>{error}</div>;
  if (!data || !data.list || data.list.length === 0)
    return <div>No weather data available</div>;

  const cityName = data.city?.name || "Unknown City";
  const currentWeather = data.list[0];
  const temperature = Math.round(currentWeather.main.temp - 273.15);
  const weatherDescription =
    currentWeather.weather[0]?.description || "Unknown";

  return (
    <Fragment>
      <h1 className="text-center text-2xl font-bold uppercase">Weather App</h1>
      <div className="relative w-fit flex items-center justify-center">
        <div className="relative w-[220px] flex flex-col items-center justify-between p-5 rounded-lg backdrop-blur-[30px] bg-[rgba(65,65,65,0.308)] border border-[rgba(255,255,255,0.089)] cursor-pointer">
          <p className="font-bold text-xl tracking-[1.2px] text-white">
            {cityName}
          </p>
          <p className="text-white text-center text-sm">
            {new Date().toLocaleTimeString()}
          </p>
          <p className="font-medium text-base tracking-[1.2px] text-[rgb(197,197,197)]">
            {weatherDescription}
          </p>
          <svg
            className="weather"
            version="1.1"
            id="Layer_1"
            x="0px"
            y="0px"
            width="100px"
            height="100px"
            viewBox="0 0 100 100"
            xmlSpace="preserve"
          >
            <image
              id="image0"
              width="100"
              height="100"
              x="0"
              y="0"
              href="data:image/png;base64,"
            ></image>
          </svg>
          <p className="temp text-white text-center text-[1.5em] font-bold">
            {temperature}°
          </p>
          <div className="minmaxContainer flex items-center justify-between gap-4 text-white">
            <div className="min">
              <p className="minHeading">Min</p>
              <p className="minTemp">{temperature - 2}°</p>
            </div>
            <div className="max">
              <p className="maxHeading">Max</p>
              <p className="maxTemp">{temperature + 2}°</p>
            </div>
          </div>
        </div>
      </div>
    </Fragment>
  );
};

export default Weather;
